...
if (tmpUser) 
	...
	if(strcasecmp(st[1],tmpUser->getUserName()) && !(target->getMode(iClient::MODE_SERVICES)))
        ...
        return true;
			...
			{
			bot->Notice(theClient,"I cant find %s anywhere!",tmpUser->getUserName().c_str());
			return false;
			}
		}
	}
...
/*
 * Find the user record, confirm authorisation and kill the other one
 */
if( st.size() < 3 )
	{
	Usage(theClient);
...
if(tmpClient && !(tmpClient->getMode(iClient::MODE_SERVICES)))
	...
	{
	bot->Notice(theClient,"I cant find %s anywhere!",st[1].c_str());
	return false;
	}
